####
# Following instructions from https://cloud.google.com/docs/terraform/resource-management/store-state for setting up remote tf backend
####
...
resource "google_storage_bucket" "remotetf" {
...
resource "local_file" "default" {
  ...
  terraform {
    backend "gcs" {
      bucket = "${google_storage_bucket.remotetf.name}"
    }
  }
  EOT
}
