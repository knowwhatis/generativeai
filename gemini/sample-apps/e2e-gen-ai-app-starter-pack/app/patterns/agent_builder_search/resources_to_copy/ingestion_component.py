...
def ingest_into_datatore(project_id:str,region_vertex:str,region_search:str,input_bucket:str,data_store_id:str) -> str:
    ...
    def add_embeddings(docs: List[Document], embedding: Embeddings) -> List[Document]:
        embeddings = embedding.embed_documents([f"{document.metadata['title']}\n{document.page_content}" for document in docs])
    ...
    def convert_docs_to_jsonl(docs, bucket_name, file_name):
        ...
        with blob.open("w") as f:
            for doc in docs:
                ...
                dictionary = {"id": doc.metadata['id'], "json_data": json.dumps(json_data)}
    ...
    def main(project_id:str,region_vertex:str,region_search:str,input_bucket:str,data_store_id:str):
        ...
        client_options = ClientOptions(api_endpoint=f"{region_search}-discoveryengine.googleapis.com")
        vertexai.init(project=project_id, location=region_vertex)
        embedding = VertexAIEmbeddings(model_name=EMBEDDING_MODEL, location=region_vertex)
