...
pipeline_name = os.getenv("PIPELINE_NAME", "No pipeline name set")
...
if __name__ == "__main__":
    ...
    pipeline_job_schedule = aiplatform.PipelineJobSchedule(
        pipeline_job=job,
        display_name=f"{pipeline_name} Weekly Ingestion Job",
    ...
    schedule_list = pipeline_job_schedule.list(
        filter=f'display_name="{pipeline_name} Weekly Ingestion Job"',
        project=project_id,
        location=region_vertex,
    )
    ...
    if not schedule_list:
        ...
        # In case that multiple schedules are created during testing, you may delete them using the following code
